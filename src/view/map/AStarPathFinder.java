...
import view.map.ClosestHeuristic;
...
public class AStarPathFinder implements PathFinder {
    ...
    public AStarPathFinder(TileBasedMap map, int maxSearchDistance,
        ...
        nodes = new Node[((GameMap)(map)).getWidth()][((GameMap)(map)).getHeight()];
        for (int x=0;x<((GameMap)(map)).getWidth();x++) {
            for (int y=0;y<((GameMap)(map)).getHeight();y++) {
    ...
    protected boolean isValidLocation(Mover mover, int sx, int sy, int x, int y) {
        boolean invalid = (x < 0) || (y < 0) || (x >= ((GameMap)(map)).getWidth() || (y >= ((GameMap)(map)).getHeight()));
